...
- 优化管理员面板按钮显示逻辑：会根据当前子话题绑定用户的拉黑状态，动态显示“拉黑用户”或“解除拉黑”。
- 调整管理员面板用户管理逻辑：`重置用户`会保留当前子话题继续复用，`删除用户并删除话题`才会删除当前绑定关系和子话题。
- 补充管理员面板作用范围说明：拉黑、解除拉黑、重置用户、删除用户并删除话题针对当前子话题绑定用户；验证码开关、用户 Raw 开关、黑名单查询为全局功能。
...
5. **管理员面板功能**  
     ...
     - 面板会根据当前子话题绑定用户的状态，动态显示“拉黑用户”或“解除拉黑”，不会同时显示两个相反操作。
     - 支持拉黑当前用户、解除拉黑、重置当前用户、删除当前用户并删除当前话题、查询黑名单、全局开关验证码、全局开关用户 Raw 等多种功能。
...
- 拉黑相关按钮会根据当前子话题绑定用户的状态动态切换：未拉黑时显示`拉黑用户`，已拉黑时显示`解除拉黑`。
- `拉黑用户`、`解除拉黑`、`重置用户`、`删除用户并删除话题`：仅作用于当前子话题绑定的那位用户。
- `重置用户`：清空当前用户的状态和消息频率记录，但保留当前子话题映射，用户下次发消息仍会回到原来的子话题。
- `删除用户并删除话题`：清空当前用户状态后，删除当前子话题；用户下次发消息时会重新创建新的子话题。
- `查询黑名单`：查询当前所有被拉黑的用户，并以 `UserID | 昵称链接` 的形式显示，属于全局操作。
- `开启/关闭验证码`：全局开关，影响所有用户的新验证流程。
- `切换到自定义问答` / `切换到数学题`：全局切换验证模式；如果还没设置自定义问答的题目和答案，则不会切换，并提示先完成配置。
