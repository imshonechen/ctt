...
- 优化管理员面板按钮显示逻辑：会根据当前子话题绑定用户的拉黑状态，动态显示“拉黑”或“解黑”。
- 调整管理员面板用户管理逻辑：`重置`会保留当前子话题继续复用，`删用户+话题`才会删除当前绑定关系和子话题。
- 补充管理员面板作用范围说明：拉黑、解黑、重置、删用户+话题针对当前子话题绑定用户；验证码开关、用户 Raw 开关、黑名单查询为全局功能。
- 新增自定义问答验证模式，支持通过管理员命令录入题目和答案，并在管理员面板中一键切换数学题验证与自定义问答验证。
...
5. **管理员面板功能**  
     ...
     - 面板会根据当前子话题绑定用户的状态，动态显示“拉黑”或“解黑”，不会同时显示两个相反操作。
     - 支持拉黑当前用户、解黑、重置、删用户+话题、黑名单、开关验证、切换验证模式、开关用户 Raw 等多种功能。
     - 支持管理员直接用用户名或用户ID执行快速拉黑/解除拉黑：`/block`、`/unblock`、`/unban`、`/ban`、`/spam`，也支持不带斜杠的 `block`、`unblock`、`unban`、`ban`、`spam` 写法。
     - 支持管理员在子话题中使用`/reset_user <chat_id>`重置指定用户状态，并保留原有子话题映射。
...
- 管理员面板已按功能重新分组：先显示当前用户操作，再显示危险操作，最后显示全局开关和辅助功能。
- 拉黑相关按钮会根据当前子话题绑定用户的状态动态切换：未拉黑时显示`拉黑`，已拉黑时显示`解黑`。
- `拉黑`、`解黑`、`重置`、`删用户+话题`：仅作用于当前子话题绑定的那位用户。
- `重置`：清空当前用户的状态和消息频率记录，但保留当前子话题映射，用户下次发消息仍会回到原来的子话题。
- `删用户+话题`：清空当前用户状态后，删除当前子话题；用户下次发消息时会重新创建新的子话题。
- `黑名单`：查询当前所有被拉黑的用户，并以 `UserID | 昵称链接` 的形式显示，属于全局操作。
- `开验证` / `关验证`：全局开关，影响所有用户的新验证流程。
- `切到问答` / `切到数学`：全局切换验证模式；如果还没设置自定义问答的题目和答案，则不会切换，并提示先完成配置。
- `开 Raw` / `关 Raw`：全局开关，影响所有用户端欢迎内容中的 Raw 链接。
- 面板顶部会显示一行简要状态：当前用户 ID、验证开关、验证模式、Raw 开关。
- `/reset_user <chat_id>`：管理员命令，用于重置指定用户的状态和消息记录，但保留原有话题映射。
- `/block <用户名|用户ID>`：直接拉黑指定用户。
- `/unblock <用户名|用户ID>`：直接解除拉黑指定用户。
- `/unban <用户名|用户ID>`：`/unblock` 的快捷别名。
- `/ban <用户名|用户ID>`、`/spam <用户名|用户ID>`：`/block` 的快捷别名。
- 以上拉黑命令也支持不带斜杠的写法，例如 `block @username`、`unblock 123456789`、`unban username`。
- 以上快速命令既可在主群中使用，也可在任意子话题中使用。
- 按用户名操作时，仅支持机器人已经见过并记录过的用户；如果没匹配到，请直接使用用户ID。
- 自定义问答验证可通过以下管理员命令录入内容：
- `/set_verify_question <题目内容>`：设置自定义问答题目。
- `/set_verify_answer <答案内容>`：设置自定义问答答案。
- 重复执行 `/set_verify_question` 或 `/set_verify_answer` 时，新内容会直接覆盖旧内容。
- 新配置只影响后续新发出的验证题，已经发出去、正在等待作答的旧验证题不会自动改变。
- `/show_verify_config`：查看当前验证模式、题目和答案配置状态。
